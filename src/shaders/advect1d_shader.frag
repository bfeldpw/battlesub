...
void main()
{
    ...
    // Neighbour values
    float v_tl = texelFetch(u_tex_buffer, ivec2(n.xy), 0).x; // top left
    float v_tr = texelFetch(u_tex_buffer, ivec2(n.zy), 0).x; // top right
    float v_bl = texelFetch(u_tex_buffer, ivec2(n.xw), 0).x; // bottom left
    float v_br = texelFetch(u_tex_buffer, ivec2(n.zw), 0).x; // bottom right
