...
void main()
{
    ...
    float r_beta = 1.0/(u_alpha+4.0+u_dissipation);
    ...
    frag_col = (u_alpha * float(texelFetch(u_tex_density_buffer, ivec2(gl_FragCoord.xy), 0).r) +
                          ...
                          float(texelFetch(u_tex_density_buffer, east, 0).r) +
                          float(texelFetch(u_tex_density_buffer, west, 0).r) +
                          float(texelFetch(u_tex_density_buffer, south, 0).r) +
                          float(texelFetch(u_tex_density_buffer, north, 0).r)
               )*r_beta;
}
